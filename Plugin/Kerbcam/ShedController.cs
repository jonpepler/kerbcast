...
namespace Kerbcam
{
    ...
    public sealed class ShedController
    {
        ...
        // 15 fps — kerbcam tolerates a slow-but-playable game and only degrades
        ...
        // Minimum fraction of frame time kerbcam must occupy before we escalate.
        // Guards the absolute-fps-target failure mode: if the game is slow for
        // reasons that AREN'T kerbcam (heavy vessel, physics), staggering can't
        ...
        /// </summary>
        /// <param name="kerbcamShare">
        /// Fraction of the current frame time attributable to kerbcam's
        /// main-thread work (0..1). Escalation is gated on this being &gt;=
        /// the controller's minEscalateShare: if kerbcam isn't a meaningful
        ...
        /// </param>
        public int Evaluate(float fpsAvg, double now, double kerbcamShare = 1.0)
        {
            double sinceChange = now - _lastChangeTime;

            // Escalate (shed) — we're overloaded at the current level AND kerbcam
            // is actually a meaningful share of frame time (else don't starve
            // feeds chasing an fps the game can't reach with kerbcam off).
            if (_level < _maxLevel && fpsAvg < _shedBelow[_level]
                && kerbcamShare >= _minEscalateShare)
            {
