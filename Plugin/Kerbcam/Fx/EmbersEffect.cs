...
// renderers with the KerbcamEmber material; the shader's geom stage
...
namespace Kerbcam
{
    internal sealed class EmbersEffect : IAtmoFxEffect
    {
        ...
        public bool TryInitialize(Camera nearCam)
        {
            ...
            _cb = new CommandBuffer { name = "Kerbcam FX Embers" };
            Debug.Log($"[Kerbcam] FX embers initialized on {nearCam.name} (geom-shader path)");
            return true;
        }
