...
- **Workflow:** `cargo build` / `cargo test` for sidecar, `pnpm -r test` for all TS packages + web page, `dotnet build` for plugin (via gonogo's `gonogo_claude_tools.sh build ocisly`).
